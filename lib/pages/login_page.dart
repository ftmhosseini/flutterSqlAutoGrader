...
import '../providers/BiometricService.dart';
...
class _LoginPageState extends State<LoginPage> {
  ...
  void initState() {
    super.initState();
    BiometricService.isAvailable().then((v) => setState(() => _biometricAvailable = v));
  }
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (userData != null) {
        ...
        await BiometricService.saveCredentials(_emailCtrl.text.trim(), _passCtrl.text);
  ...
  Future<void> _handleBiometricLogin() async {
    ...
    if (credentials == null) {
      setState(() => _error = 'No saved credentials. Please log in with password first.');
      return;
    }
    ...
    try {
      ...
      final userData = await getUser(cred.user!.uid);
      if (userData != null) {
        UserSession.set(userData);
        if (mounted) context.go('/dashboard');
      }
    } catch (_) {
      setState(() => _error = 'Biometric login failed. Please log in with password.');
    }
  }
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  autofillHints: const [AutofillHints.email], // <--- Add this hint
                ...
                TextField(
                  ...
                  obscureText: true,
                  autofillHints: const [AutofillHints.password], // <--- Add this hint
                  // onEditingComplete: () => TextInput.finishAutofillContext(), // <--- Optional but recommended
